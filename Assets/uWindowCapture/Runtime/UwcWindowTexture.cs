...
public class UwcWindowTexture : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!isValid) {
            if (material_) material_.mainTexture = null;
            if (rawImage_) rawImage_.texture = null;
            return;
        }
    ...
    void UpdateTexture()
    {
        ...
        if (material_ && material_.mainTexture != window.texture) {
            material_.mainTexture = window.texture;
        }
        if (rawImage_ && rawImage_.texture != window.texture) {
    ...
    void UpdateScale()
    {
        ...
        switch (scaleControlType) {
            case WindowTextureScaleControlType.BaseScale: {
                Vector2 extents;
                if (rectTransform_) {
                    extents = rectTransform_.rect.size*0.5f;
                } else {
                    extents = meshFilter_.sharedMesh.bounds.extents;
                }
