...
msgIn = {}
MAX_ATTEMPTS = 1000
function plunder.getServerMsg(windowName, instance)
	-- print("getting input")
	instance = instance or 0
	local ext = instance > 0 and "-"..instance or ""
	if not plunder.USE_SERVER then return end
	...
	plunder.executeInMsg(resp)
end

function plunder.executeInMsg(text)
	msgIn = json.decode(resp)
	...
	if msgIn ~= nil then
		...
		if emu.getsystemid() == "N64" then
			...
			-- hack for controllers that don't have have analog input, but i wanna keep the axis input across
			-- TODO: get pad depending on emulator
			local up = "D-Pad Up"
			local down = "D-Pad Down"
			local right = "D-Pad Right"
			local left = "D-Pad Left"


			-- TODO: maybe pwm here? have up and down percentage of frames based on x input
			joypad.set({
				[up] = y > 0,
				[down] = y < 0,
		...
		local b = msgIn["b"] or 0
		local z = msgIn["z"] or 0

		for i, m in pairs(INPUTS_MAPS) do
			local s = msgIn[m.osc] or 0
			if msgInLast[m.osc] ~= a then
				joypad.set({
					[m.emu] = s > 0,
				}, 1)
			end
		...
		local slot = math.floor(msgIn["save"] or 0)
		if slot > 0 and slot ~= math.floor(msgInLast["save"] or 0) then
			savestate.saveslot(slot)
		end

		local volume = msgIn["volume"] or -1
		if (msgInLast["volume"] or -1) ~= volume and volume >= 0 then
			client.SetVolume(volume)
		end

		slot = math.floor(msgIn["load"] or 0)
		if slot > 0 and slot ~= math.floor(msgInLast["load"] or 0) then
			savestate.loadslot(slot)
		end

		local sample = msgIn["sample"] or false

		if msgInLast["sample"] ~= sample and sample and sample ~= -1 then
			plunder.runSample(sample)
		end

		local pause = msgIn["pause"] or 0
		if (msgInLast["pause"] or 0) ~= pause and pause > 0 then
			print("pausing")
			client.pause()
		end


		local unpause = msgIn["unpause"] or 0
		if (msgInLast["unpause"] or 0) ~= unpause and unpause > 0 then
			print("unpausing")
			client.unpause()
		end
